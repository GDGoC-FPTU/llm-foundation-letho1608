...
def compare_models(prompt: str) -> dict:
    ...
    return {
        "gpt4o": {
            ...
            "cost": calc_cost(OPENAI_MODEL, gpt4o_usage["input_tokens"], gpt4o_usage["output_tokens"]),
        ...
        "gpt4o_mini": {
            ...
            "cost": calc_cost(OPENAI_MINI_MODEL, mini_usage["input_tokens"], mini_usage["output_tokens"]),
        ...
        "gemini_flash": {
            ...
            "cost": calc_cost(GEMINI_MODEL, gemini_usage["input_tokens"], gemini_usage["output_tokens"]),
...
def streaming_chatbot() -> None:
    ...
    client = genai.Client(api_key=os.getenv("GEMINI_API_KEY"))
    history = []
    print("Gemini 2.5 Flash Chatbot (type 'quit' or 'exit' to stop)")
    while True:
        user_input = input("\nYou: ")
    ...
    for item in results:
        ...
        for key, label in model_labels.items():
            if key not in item:
                continue
            stats = item[key]
            response_trunc = stats["response"][:50].replace("\n", " ")
            latency = f"{stats['latency']:.2f}s"
            tokens = f"{stats['input_tokens']}/{stats['output_tokens']}"
            cost = f"${stats['cost']:.6f}"
            lines.append(f"| {prompt} | {label} | {response_trunc} | {latency} | {tokens} | {cost} |")
    return "\n".join(lines)
